...
Production Postgres uses its own tunnel and connector, not this stack. See `templates/postgres-cf-tunnel.compose.yml` and [docs/wiki/POSTGRES-CF-TUNNEL.md](../docs/wiki/POSTGRES-CF-TUNNEL.md). `services.sh` still accepts a `postgres://` line for a database that this tunnel can reach.
